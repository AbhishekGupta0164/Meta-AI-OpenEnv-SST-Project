...
check("Response has status=ok", lambda: call("GET", "/health")["status"] == "ok")
...
check("4 tasks exist (easy/medium/hard/expert)", lambda: len(tasks) == 4)
...
def test_inference():
    ...
    assert "[END]" in result.stdout, "inference.py did not complete: " + result.stderr[:200]
...
for f in required:
    check("File exists: " + f, lambda fn=f: os.path.exists(fn))
...
if FAIL == 0:
    print("  DONE: ALL CHECKS PASSED -- READY TO DEPLOY!")
else:
    print("  FAIL: Fix " + str(FAIL) + " issues before deploying")
print("="*55 + "\n")